...
def regenerateData(pcaData,Uk,dataMean):
    ...
    np.savetxt("Data/regeneratedData."+ str(height) + "x" + str(width) + "." + str(K)+".txt",regeneratedData)
...
def display(K):
    ...
    for i in range(0,nLines,18):
        ...
        for j in range(len(Reg)):
            Reg[j] = float(Reg[j])
        for j in range(len(Reg)):
            if (Reg[j]>max) : max =Reg[j]
            if (Reg[j]<min) : min =Reg[j]
        for j in range(len(Reg)):
            temp = Reg[j]
            Reg[j] = np.uint8((temp-min)/(max-min)*255)
...
def runTestcases(K):
    ...
    try:
        ...
        [X,pcaData,Uk,dataMean] = myPCA(K=500)
